...
withSession = function(req, res){
  var sessionSecret, tryParse, that, decipher, decrypted;
  ...
  req.session = req.cookies['quinn.session']
    ? (decipher = createDecipher('aes192', sessionSecret), decrypted = decipher.update(req.cookies['quinn.session'], 'base64', 'utf8'), decrypted += decipher.final('utf8'), (function(){
      try {
  ...
  res.on('header', function(){
    var cipher, serialized, encrypted;
    if (req.session != null) {
      cipher = createCipher('aes192', sessionSecret);
      serialized = JSON.stringify(req.session);
      encrypted = cipher.update(serialized, 'utf8', 'base64');
      encrypted += cipher.final('base64');
      return res.setHeader('Set-Cookie', cookie.serialize('quinn.session', encrypted));
    } else {
