...
withSession = function(req, res){
  var sessionSecret, tryParse, that, decipher, fromCookie, decrypted, err;
  ...
  req.session = req.cookies['quinn.session']
    ? (decipher = createDecipher('aes192', sessionSecret), (function(){
      try {
        fromCookie = JSON.parse(req.cookies['quinn.session']);
        decrypted = decipher.update(fromCookie.e, 'base64', 'utf8');
  ...
  res.on('header', function(){
    var cipher, serialized, encrypted, forCookie;
    if (req.session != null) {
