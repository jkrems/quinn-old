...
var Q, pageModel, wrapGeneratorFn, target, COMMENTS_PATTERN, ARGS_PATTERN, ARG_PATTERN, targetFromFn, parseTarget, requestContext, action, inject, toString$ = {}.toString, slice$ = [].slice;
Q = require('q');
pageModel = require('./page-model');
wrapGeneratorFn = function(makeGenerator){
  return function(){
    var continuer, generator, callback, errback;
    continuer = curry$(function(verb, arg){
      ...
      try {
        ...
        if (result.done) {
          ...
          return Q.when(result.value, callback, errback);
        }
      } catch (e$) {
        exception = e$;
    ...
    if ('Generator' === toString$.call(generator).slice(8, -1)) {
...
targetFromFn = function(targetFn){
  ...
  targetFn = wrapGeneratorFn(targetFn);
