...
var glob, path, readFileSync, swig, Duplex, helpers, uuid, cacheTemplates, cacheAllowErrors, templateInfo, templates, _modules, templateError, swig__compileFile, promisedBlocks, promisedBlock, ResolveTemplateStream, whenHelper, render, addTemplate, loadTemplates, slice$ = [].slice;
glob = require('glob');
path = require('path');
...
swig.compileFile = function(templateName, forceAllowErrors){
  ...
  if (cacheAllowErrors || forceAllowErrors) {
    return get();
  } else {
    try {
      return get();
